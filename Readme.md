...
Пример публикации валют:
    ...
    $ curl -XPOST -H 'Content-Type: application/json' 'https://tradeagent.mobi/api/v1/currencies?key=YOUR_API_KEY' -d '[
        {"id": 980, "name": "Гривня", "shortName": "грн", "rate": 1.0, "factor": 1.0},
        {"id": 840, "name": "Долар", "shortName": "дол", "rate": 8.4, "factor": 1.0},
...
Групы товаров образуют многоуровневое дерево.
...
<tr>
    ...
    <td>double</td>
    <td>нет</td>
    <td>Коэффициент ПДВ (1.0 или 1.2)</td>
</tr>
<tr>
    <td>currencyId</td>
    <td>int</td>
...
<tr>
    ...
    Если нуль, то приоритет следующий: категория цены, минимальное количество товара, максимальна цена</td>
</tr>
</tbody>
</table>
...
* docs - сообщение про наявность новых необработанных документов
